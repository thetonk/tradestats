...
static bool tradeLogging = false, candleSleep = true;
...
static pthread_mutex_t tradeLoggerMutex, candleThreadMutex;
static pthread_cond_t tradeLoggerCondition, candleThreadCondition;
...
static void onSigInt(int sig)
{
	...
	pthread_cond_signal(&candleThreadCondition);
...
void *consumerCandle(void *q){
	...
	Trade last;
	while(!bExit){
		while(candleSleep && !bExit){
			//lwsl_warn("Thread sleeping, vector empty\n");
			pthread_cond_wait(&candleThreadCondition,&candleThreadMutex);
			candleSleep = false;
			if(bExit) return NULL;
		}
		if(vec->size > 0){
			//printf("Candle thread queue size: %zu\n",vec->size);
			pthread_mutex_lock(vec->mutex);
			//sort trades by timestamp ascending and remove one
			qsort(vec->data,vec->size,sizeof(Trade),tradeCompareByTime);
			...
			if ((int64_t) candles[last.symbolID].totalVolume == (int64_t) INIT_VOLUME_VALUE){
				...
				Candle finalCandle;
				for(size_t i = 0; i < symbolCount; ++i){
					finalCandle = candles[i];
					printf("[LAST MINUTE CANDLE %s] max: %.2lf min: %.2lf first: %.2lf, last %.2lf total volume: %lf\n",Symbols[i],finalCandle.max.price,finalCandle.min.price,finalCandle.first.price,finalCandle.last.price,finalCandle.totalVolume);
					writeCandleFile(Symbols[i], &finalCandle);
...
void *ticker(void *arg){
	...
	struct tm dateStart, dateLast;
	timeNow = time(NULL);
	dateLast = *localtime(&timeNow);
	dateStart = *localtime(&timeNow);
	while(!bExit){
		timeNow = time(NULL);
		dateLast = *localtime(&timeNow);
		if(dateLast.tm_min == dateStart.tm_min+1){
			printf("[TICKER] Candle tick!\n");
			dateStart = dateLast;
			pthread_cond_signal(&candleThreadCondition);
		}
		sleep(3);
...
int main(int argc, char *argv[])
{
	...
	pthread_mutex_init(&candleThreadMutex,NULL);
	pthread_cond_init(&candleThreadCondition,NULL);
	...
	pthread_cond_destroy(&candleThreadCondition);
	pthread_mutex_destroy(&candleThreadMutex);
